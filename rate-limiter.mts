...
export class RateLimiter {
  ...
  async add(ids: string[]) {
    ...
    // Optimization: Pre-allocate args array using Array(size) which is faster in V8 than setting .length on an empty array.
  ...
  static async addAndCheckWindows(
    ...
    const keys: string[] = Array(len);
    const args: string[] = Array(len * 4 + 1);
